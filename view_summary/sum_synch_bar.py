...
def make_synch_bar(df):
   ...
   #Clean the df inside the function to allow responsive updates to filtering/brushing
    ...
    # true where concordance is at or above threshold
    ...
    data = pd.DataFrame({
        "Frequency": [
            "High Frequency Concordance", "High Frequency Concordance",
            "Low Frequency Concordance",  "Low Frequency Concordance",
        ],
    ...
    fig = px.bar(
        ...
        category_orders={
            "Leader": ["Child", "Parent"],
            "Frequency": ["High Frequency Concordance", "Low Frequency Concordance"],
        },
        color_discrete_map={
    ...
    fig.update_layout(
        title="Concordance Moments Led by Each Participant",
        showlegend=False,
