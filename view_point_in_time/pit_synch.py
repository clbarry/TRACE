...
# data paths and col names, this will eventually be dynamically set
TS_COL = "timestamp"
LF_COL = "lf_coh"
HF_COL = "hf_coh"
...
def half_donut_segments(v):
    ...
    v = float(np.clip(v, 0.0, 1.0)) # coh value
...
def make_concordance_figure(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Pie(
            ...
            sort=False,
            textinfo="none",  # don't show labels for background
            showlegend=False,
            hoverinfo="skip",
            domain=dict(x=[0, 1], y=[GLYPH_Y_MIN, GLYPH_Y_MAX]),  # sizing
        )
    )
